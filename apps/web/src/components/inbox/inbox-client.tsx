...
export function InboxClient({ received, sent, users, currentUserId, canCompose = true }: InboxClientProps) {
  ...
  return (
    <div className="flex bg-white border border-border rounded-2xl h-full overflow-hidden relative">
      ...
      {/* 1. LEFT SIDEBAR */}
      <aside className="w-64 border-r border-border bg-slate-50/50 flex flex-col shrink-0 p-4 space-y-4 hidden md:flex">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        {/* Mobile Folder & Category Bar (2 Rows, No Horizontal Scroll) */}
        <div className="flex md:hidden flex-col gap-2 p-3 border-b border-border shrink-0 bg-slate-50/50">
          {/* Row 1: Compose, Inbox, Sent */}
          <div className="flex items-center gap-2 w-full">
            {canCompose && (
              <button
                ...
                className="flex items-center justify-center gap-1 px-2 py-2 bg-blue-600 text-white rounded-lg text-xs font-semibold flex-1 shadow-md shadow-blue-600/10 active:bg-blue-700 transition-colors"
              >
            ...
            <button
              ...
              className={`px-2 py-2 rounded-lg text-xs font-semibold flex-1 text-center transition-colors ${
                activeFolder === "inbox" ? "bg-blue-100 text-blue-700 font-bold" : "bg-slate-100 text-muted-foreground"
              }`}
            >
            ...
            <button
              ...
              className={`px-2 py-2 rounded-lg text-xs font-semibold flex-1 text-center transition-colors ${
                activeFolder === "sent" ? "bg-blue-100 text-blue-700 font-bold" : "bg-slate-100 text-muted-foreground"
              }`}
            >
              Sent
            </button>
          ...
          {activeFolder === "inbox" && (
            <div className="flex items-center gap-2 w-full">
              <button
                ...
                className={`px-2 py-1.5 rounded-lg text-[11px] font-semibold flex-1 text-center transition-colors ${
                  activeCategory === "PRIMARY" ? "bg-slate-800 text-white font-bold" : "bg-slate-100 text-muted-foreground"
                }`}
              >
                Primary {categoryUnreadCounts.PRIMARY > 0 && `(${categoryUnreadCounts.PRIMARY})`}
              </button>
              ...
              <button
                ...
                className={`px-2 py-1.5 rounded-lg text-[11px] font-semibold flex-1 text-center transition-colors ${
                  activeCategory === "UPDATES" ? "bg-slate-800 text-white font-bold" : "bg-slate-100 text-muted-foreground"
                }`}
              >
                Updates {categoryUnreadCounts.UPDATES > 0 && `(${categoryUnreadCounts.UPDATES})`}
              </button>
              ...
              <button
                ...
                className={`px-2 py-1.5 rounded-lg text-[11px] font-semibold flex-1 text-center transition-colors ${
                  activeCategory === "SOCIAL" ? "bg-slate-800 text-white font-bold" : "bg-slate-100 text-muted-foreground"
                }`}
              >
                Social {categoryUnreadCounts.SOCIAL > 0 && `(${categoryUnreadCounts.SOCIAL})`}
              </button>
            </div>
          )}
        </div>
            ...
            {filteredEmails.length === 0 ? (
              ...
              <div className="divide-y divide-border border-b border-border">
                 {filteredEmails.map(email => {
                  const isUnread = activeFolder === "inbox" && !email.read;
                  return (
                    <div key={email.id} onClick={() => handleSelectEmail(email)}>
                      {/* DESKTOP VIEW ROW */}
                      <div className={`hidden md:flex items-center gap-4 px-6 py-3 cursor-pointer transition-colors text-sm hover:bg-slate-50/50 ${
                        isUnread ? "bg-blue-50/30 font-semibold text-slate-900 border-l-2 border-blue-500" : "text-slate-600"
                      }`}>
                        {/* Unread indicator */}
                        ...
                        <div className="flex-1 min-w-0 flex items-baseline gap-2">
                          <span className={`${isUnread ? "text-slate-900" : "text-slate-800"}`}>
                            {email.subject}
                          </span>
                      ...
                      {/* MOBILE VIEW ROW */}
                      <div className={`flex md:hidden flex-col gap-1.5 px-4 py-3.5 cursor-pointer transition-colors text-sm hover:bg-slate-50/50 ${
                        isUnread ? "bg-blue-50/20 font-semibold text-slate-900 border-l-2 border-blue-500" : "text-slate-600"
                      }`}>
                        ...
                        <div className="flex items-center justify-between w-full">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            {email.system && (
                              <span className="px-1 py-0.2 bg-amber-100 text-amber-800 border border-amber-200 rounded text-[9px] uppercase font-bold shrink-0">
                                System
                              </span>
                            )}
                        ...
                        {/* Row 2: Subject */}
                        <div className={`text-slate-900 text-xs truncate ${isUnread ? "font-semibold" : "font-medium"}`}>
                          {email.subject}
                        </div>
      ...
      {isComposing && (
        <div className="fixed bottom-0 right-0 md:right-12 w-full md:w-[500px] h-full md:h-auto max-h-[100dvh] md:max-h-[85vh] bg-white border-t md:border border-border rounded-t-none md:rounded-t-2xl shadow-2xl z-50 overflow-hidden flex flex-col animate-in slide-in-from-bottom duration-300">
          ...
          {/* Form */}
          <form onSubmit={handleSend} className="p-4 flex-1 overflow-y-auto space-y-3">
            ...
            {showRecipientsDirectory && (
              <div className="absolute right-2 md:right-12 top-20 w-[calc(100%-16px)] md:w-64 bg-white border border-border rounded-lg shadow-premium z-[60] overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
